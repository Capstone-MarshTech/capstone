...
export const openCount = async (req, res) => {
    ...
    try { 
        ...
        const open_claims_count = await Claim.countDocuments({ open_claim: true, cleansed_policyyear: year }); 
...
export const distinctClaimsCountByYear = async (req, res) => {
    const  year  = parseInt(req.params.year)
    try { 
        const distinct_claims = await Claim.distinct("claim_number", {cleansed_policyyear: year }); 
