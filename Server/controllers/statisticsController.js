...
export const averageTotalIncurredByPolicyYearbyLossBanding = async (
  req,
  res
) => {
  const loss_band = req.query.loss_banding;
...
export const lossBandingValuesByYear = async (req, res) => {
  ...
  try {
    ...
    const distinct_loss_banding_values_ordered =
      distinct_loss_banding_values.sort((bandingString1, bandingString2) => {
        ...
        if (
          bandingStringArray1.length >= 3 &&
          bandingStringArray2.length >= 3
        ) {
